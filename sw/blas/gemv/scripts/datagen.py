#!/usr/bin/env python3
# Copyright 2024 ETH Zurich and University of Bologna.
# Licensed under the Apache License, Version 2.0, see LICENSE for details.
# SPDX-License-Identifier: Apache-2.0
#
# Authors: Luca Colagrande <colluca@iis.ee.ethz.ch>

import numpy as np
import os
import sys

sys.path.append(os.path.join(os.path.dirname(__file__), "../../../../util/sim/"))
from data_utils import DataGen, format_array_declaration, format_struct_definition, \
                       format_array_definition, format_ifdef_wrapper  # noqa: E402


np.random.seed(42)


class GemvDataGen(DataGen):

    # AXI splits bursts crossing 4KB address boundaries. To minimize
    # the occurrence of these splits the data should be aligned to 4KB
    BURST_ALIGNMENT = 4096

    def golden_model(self, alpha, a, x):
        return alpha * np.matmul(a, x).flatten()

    def emit_header(self, **kwargs):
        header = [super().emit_header()]

        m, n, alpha = kwargs['m'], kwargs['n'], kwargs['alpha']

        a = np.random.rand(m, n)
        x = np.random.rand(n, 1)
        y = self.golden_model(alpha, a, x)

        # Store matrix in transposed form if requested
        a = a.T if kwargs['trans'] else a

        a_uid = 'a'
        x_uid = 'x'
        y_uid = 'y'

        cfg = {
            **kwargs,
            'a': a_uid,
            'x': x_uid,
            'y': y_uid,
        }

        a = a.flatten()
        x = x.flatten()

        header += [format_array_declaration('double', a_uid, a.shape)]
        header += [format_array_declaration('double', x_uid, x.shape)]
        header += [format_array_declaration('double', y_uid, y.shape)]
        header += [format_struct_definition('gemv_args_t', 'args', cfg)]
        header += [format_array_definition('double', a_uid, a,
                                           section=kwargs['section'])]
        header += [format_array_definition('double', x_uid, x,
                                           section=kwargs['section'])]
        result_def = format_array_definition('double', 'result', y)
        header += [format_ifdef_wrapper('BIST', result_def)]
        header = '\n\n'.join(header)

        return header


if __name__ == "__main__":
    sys.exit(GemvDataGen().main())
