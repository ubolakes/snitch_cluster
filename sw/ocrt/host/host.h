...
static inline void delay_ns(uint64_t delay);
...
static volatile uint32_t* const fll_system_base =
    (volatile uint32_t*)FLL_SYSTEM_BASE_ADDR;
static volatile uint32_t* const fll_periph_base =
    (volatile uint32_t*)FLL_PERIPH_BASE_ADDR;
static volatile uint32_t* const fll_hbm2e_base =
...
// static volatile uint32_t* const fll_base[N_CLOCKS] = {fll_system_base,
// fll_periph_base, fll_hbm2e_base};

static volatile uint64_t* const clint_mtime_ptr =
    (volatile uint64_t*)(CLINT_BASE_ADDR + CLINT_MTIME_LOW_REG_OFFSET);
static volatile uint64_t* const clint_mtimecmp0_ptr =
...
static volatile comm_buffer_t comm_buffer __attribute__((aligned(8)));
...
static inline void set_d_cache_enable(uint16_t ena) {
...
static inline void wait_snitches_parked(uint32_t timeout) { delay_ns(100000); }
 ...
 */
static inline void wakeup_snitches_selective(uint32_t base_hartid, uint32_t num_harts, uint32_t stride) {
    volatile uint32_t* lock = get_shared_lock();
 ...
 *         avoiding congestion.
 */
static inline void wakeup_master_snitches() {
    volatile uint32_t* lock = get_shared_lock();
...
static inline void set_cluster_interrupt(uint32_t cluster_id, uint32_t core_id) {
...
static inline void wait_timer_interrupt() {
    do
        wfi();
...
static inline void disable_timer_interrupts() {
    uint64_t mie;
...
static inline void disable_sw_interrupts() {
    uint64_t mie;
 ...
 *         interconnect and shared CLINT.
 */
static inline void wait_sw_interrupt_cleared() {
    while (sw_interrupt_pending())
        ;
}
 ...
 *         of the shared CLINT.
 */
static inline void wait_remote_sw_interrupt_pending(uint32_t hartid) {
...
static inline void set_timer_interrupt(uint64_t interval_ns) {
 ...
 */
static inline void clear_timer_interrupt() { *clint_mtimecmp0_ptr = mtime() + 1; }
...
static uint32_t const ISO_MASK_ALL = 0b1111;
static uint32_t const ISO_MASK_NONE = 0;
...
static inline void isolate_quad(uint32_t quad_idx, uint32_t iso_mask) {
    *quad_cfg_isolate_ptr(quad_idx) |= iso_mask;
    fence();
}
 ...
 */
static inline uint32_t check_isolated_timeout(uint32_t max_tries, uint32_t quadrant_idx,
                                uint32_t iso_mask) {
...
static inline void activate_interleaved_mode_hbm() {
    uint64_t addr =
        OCCAMY_HBM_XBAR_INTERLEAVED_ENA_REG_OFFSET + HBM_XBAR_CFG_BASE_ADDR;
    *((volatile uint32_t*)addr) = 1;
}

static inline void deactivate_interleaved_mode_hbm() {
    uint64_t addr =
        OCCAMY_HBM_XBAR_INTERLEAVED_ENA_REG_OFFSET + HBM_XBAR_CFG_BASE_ADDR;
    *((volatile uint32_t*)addr) = 1;
}
